...
from analog_gauge_reader.srv import (GaugeReader as GaugeReaderSrv,
                                     GaugeReaderRequest, GaugeReaderResponse)
...
class AnalogGaugeReaderRos:
    ...
    def __init__(self, debug=False):
        ...
        self.segmentation_model_path = rospy.get_param(
        ...
        self.publishers = {}
        self.readings_pub = rospy.Publisher("~readings",
                                            GaugeReadings,
                                            queue_size=1,
                                            latch=self.latch)
    ...
    def read(self, _req: GaugeReaderRequest):
        ...
        if original_image is None:
            ...
            raise rospy.ServiceException(
                "No image received yet, ignoring read request")
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as out_path:
                ...
                try:
                    ...
                    for i in range(2) if len(
                            self.publishers.keys()) <= 1 else [1]:
                        for visual_result in os.listdir(out_path):
                            visual_result_split = os.path.splitext(
                                visual_result)
                            if visual_result_split[1] != ".jpg":
                                continue
                            visual_result_image = cv2.imread(
                                os.path.join(out_path, visual_result))
                            self.visualize(visual_result_image, original_image,
                                           visual_result_split[0], i == 0)
                        if i == 0:
                            rospy.sleep(3)
            ...
            for gauge_reading in gauge_readings:
                ...
                value = Float64()
                value.data = gauge_reading[
                    "value"] if self.round_decimals < 0 else round(
                        gauge_reading["value"], self.round_decimals)
                unit = String()
                unit.data = gauge_reading["unit"] if gauge_reading[
    ...
    def visualize(self, img, original_image, name, create_only=False):
        if name not in self.publishers:
            ...
            self.publishers[name] = rospy.Publisher(f"~{name}",
                                                    Image,
                                                    queue_size=1,
                                                    latch=self.latch)
            if not create_only:
